...
public class MonticuloMax <Anytype extends Comparable<Anytype>>{
    ...
    public MonticuloMax<Anytype> add(Anytype data){
        if(isFull()){
    ...
    private void resize(int capacidad){
        ...
        //es donde va a empezar a agregar el proximo array (creo nose esta re loco)
    }
    ...
    public Anytype buscar(Anytype data) throws Exception {
        for (int i = 0; i < monticulo.length; i++) {
            if(data == monticulo[i]) return data;
        }
